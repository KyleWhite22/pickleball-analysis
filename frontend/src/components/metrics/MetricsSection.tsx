...
function KPIRow() {
  ...
  const totalMatches = standings ? Math.floor(standings.reduce((s,p)=>s+p.wins+p.losses,0)/2) : 0;
  const avgWinPct = standings && standings.length ? (standings.reduce((s,p)=>s+p.winPct,0)/standings.length)*100 : 0;
...
export default function MetricsSection({ leagueId }: { leagueId: string | null }) {
  return (
    <MetricsProvider leagueId={leagueId}>
